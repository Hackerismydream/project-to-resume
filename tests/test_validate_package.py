...
class ValidatePackageTest(unittest.TestCase):
    ...
    def test_eval_questions_are_capped_at_three(self) -> None:
        ...
        path.write_text(json.dumps(data, ensure_ascii=False, indent=2), encoding="utf-8")
        errors = VALIDATOR.validate_tree(package)
        self.assertTrue(any("schema violation" in error and "expected_questions" in error for error in errors), errors)
    ...
    def test_generated_artifact_is_rejected(self) -> None:
        ...
        errors = VALIDATOR.validate_tree(package)
        self.assertTrue(any("generated" in error and "__pycache__" in error for error in errors), errors)
