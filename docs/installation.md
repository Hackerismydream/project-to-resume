...
`project-to-resume` 是一个目录型 Agent Skill，不是需要启动服务的 Python 包。安装的核心是让 Coding Agent 能够读取 `SKILL.md`、`references/` 和相关示例。
...
CLI 会读取 Skill frontmatter，并将目录安装到受支持 Agent 的技能位置。不同 Agent 的最终目录可能不同；通用的项目级规范位置是：

```text
.agents/skills/project-to-resume/
```

安装时只选择你实际使用的 Agent，避免在项目中生成无关配置目录。
...
某些 Agent 使用自己的技能目录。此时可以复制该目录，或从 Agent 目录建立指向 `.agents/skills/project-to-resume` 的符号链接。以对应 Agent 的官方文档为准。
...
使用 CLI 安装时，可以重新执行安装命令并确认覆盖目标 Skill。手动安装时，先保留自己的本地修改，再用新的完整目录替换旧目录：
...
确认目录名是 `project-to-resume`，根目录直接包含 `SKILL.md`，且 frontmatter 中的 `name` 同样是 `project-to-resume`。重新启动或刷新 Agent 的 Skill 索引。

### 只得到了通用润色

明确要求“先读取当前仓库”，并确认 Agent 拥有仓库读取权限。对于很大的仓库，可以提供固定 commit、主要入口或希望投递的岗位，但不要先指定必须找到某个技术关键词。
...
正常。Skill 的按需 references、示例和评测合同属于可分发内容。宿主 Agent 应只在任务需要时加载相关文件。
